...
/**
 * Paystack card checkout — Paystack's hosted `card` channel (Visa / Mastercard,
 * charged in KES). Same gateway as the "M-Pesa" button, different channel.
 * Renders an unavailable notice until PAYSTACK_SECRET_KEY is configured.
 */
export function PaystackCardSection() {
  ...
  const onPay = async () => {
    setSubmitting(true);
    try {
      const result = await startCheckout({ tier: "basic", channel: "card" });
      if (!result?.authorizationUrl) {
        throw new Error(t("payment.pscardFailed"));
      }
      window.location.href = result.authorizationUrl;
    } catch (error) {
      toast.error(getSafeUserError(error, t("payment.pscardFailed")));
      setSubmitting(false);
    }
  };
          ...
          >
            {submitting ? (
              <>
                ...
                {t("payment.pscardRedirecting")}
              </>
            ) : (
              t("payment.pscardPay", { price: priceLabel })
            )}
          </Button>
        )}
        ...
        <div className="flex items-start gap-2.5 rounded-2xl border border-border/70 bg-muted/30 px-4 py-3 text-xs leading-relaxed text-muted-foreground">
          ...
          <p>
            <span className="font-medium text-foreground/80">
              {t("payment.pscardSecureNote")}
            </span>{" "}
            {t("payment.pscardStepsNote")}
          </p>
        </div>
      </div>
    </section>
