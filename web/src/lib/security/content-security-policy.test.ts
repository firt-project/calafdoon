...
describe("buildContentSecurityPolicy (L2)", () => {
  it("production CSP is present and blocks plugins / framing / base hijack", () => {
    ...
    assert.doesNotMatch(csp, /tel-calafkaaga-1\.onrender\.com/);
  ...
  it("absolute api.helcalafkaaga.com is allowed in connect-src when configured", () => {
    const csp = buildContentSecurityPolicy({
      NODE_ENV: "production",
    ...
    assert.match(
      csp,
      /connect-src[^;]*https:\/\/api\.helcalafkaaga\.com/
    );
    assert.match(
      csp,
      /connect-src[^;]*wss:\/\/api\.helcalafkaaga\.com/
    );
  });
