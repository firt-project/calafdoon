/**
 * Lucia-compatible Scrypt verifier (Phase 1 — no login endpoint).
 * Keep in sync with packages/migration/src/crypto/lucia-scrypt.ts
 */
import {
  createHash,
  randomBytes,
...
export async function verifyPasswordLuciaScrypt(
  ...
  try {
    ...
    const actual = await generateScryptKey(password.normalize("NFKC"), saltHex);
