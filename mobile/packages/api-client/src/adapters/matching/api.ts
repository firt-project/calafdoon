...
export const apiMatching: MatchingAdapter = {
  async getMatches(filters, signal?: AbortSignal) {
    try {
      ...
      } | unknown>(`/matches/discover${toQuery(filters)}`, { signal });
      // Nest returns { items, nextCursor }; Convex-style array — normalize.
      if (
        res &&
        typeof res === "object" &&
        "items" in res &&
        ...
        const page = res as { items: unknown[]; nextCursor?: string | null };
        return {
          items: page.items,
      ...
      if (isPaidGateError(error)) return { items: [], nextCursor: null };
      throw error;
    }
  },
  ...
  async getPeerCard(userId) {
    try {
      return await apiClient.get(
        `/matches/${encodeURIComponent(userId)}/card`
      );
    } catch (error) {
      if (isPaidGateError(error)) return null;
      throw error;
